...
import {
  ...
  MockMessageBusAdapter,
  RobloxDataStoreAdapter,
  RobloxMessageBusAdapter,
  sameSession,
...
class VersionQuery<T extends object> {
  ...
  public constructor(
    private readonly store: FossixStore<T>,
...
export class FossixStore<T extends object> {
  ...
  public readonly OnError = new Signal<
  ...
  public async StartSessionAsync(
    ...
    while (elapsed <= this.config.startSessionTimeoutSeconds) {
      ...
      const delay = firstTry
        ? this.config.firstLoadRepeatSeconds
        : this.config.loadRepeatPeriodSeconds;
      firstTry = false;
      task.wait(delay);
  ...
  private async tryAcquireSession(
    profileKey: string,
    allowSteal: boolean,
  ): Promise<Profile<T> | undefined> {
    try {
      ...
      await this.keyQueue.enqueue(profileKey, async () => {
        const updated = await this.dataStoreAdapter.updateAsync(profileKey, (current) => {
          ...
          const owner = doc.metaData.activeSession;
          const stale =
            owner !== undefined && now - doc.metaData.lastUpdated >= this.config.assumeDeadSeconds;
