...
import { IDataStoreAdapter, IMessageBusAdapter, SessionTag, StoredProfileDocument } from '../types';
  ...
  public async updateAsync(
    ...
    return new Promise((resolve, reject) => {
      const [ok, result] = pcall(() =>
        this.store.UpdateAsync(key, (current: unknown) =>
          transform(current as StoredProfileDocument),
        ),
      );
      if (!ok) {
        reject(result);
...
export class RobloxMessageBusAdapter implements IMessageBusAdapter {
  public async publish(profileKey: string, payload: unknown): Promise<void> {
    return new Promise((resolve, reject) => {
      const [ok, result] = pcall(() =>
        MessagingService.PublishAsync(topicName(profileKey), payload),
      );
      if (!ok) {
        reject(result);
        return;
